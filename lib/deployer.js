...
class Deployer {
  ...
  renameThemeFile () {
    ...
    // max 12 chars
    let author = ''
    exec('git log -1 --pretty=format:%an', (err, stdout) => {
      if (err) {
        console.error(err)
        return
      }
      ...
      author = arr.length >= 2 ? `${arr[0]} ${arr[arr.length-1].substr(0,1)}` : arr[0]
    })

    this.getCurrentCommit().then(commit => {
      let authorName = author == "" ? "Automated" : author.substr(0,12)

      const data = {
        "theme": {
          "id": config.get('theme_id'),
          "name": `${themeNames[config.get('env')]} - v${version} (${commit.substr(0,7)}) - ${authorName} - ${timestr})`
        }
      }
