...
class Gulper {
  ...
  build (upload = false, files = false) {
    return new Promise((resolve, reject) => {
      ...
      const first = ~path.indexOf('assets/svg')
        ? this.icons({path}, upload)
        : this.copy({type, path}, upload)

      const chain = matches.reduce((prev, path) => {
        return ~path.indexOf('assets/svg')
          ? prev.then(this.icons.bind(this, {path}, upload))
  ...
  icons ({path}, upload = false) {
    return new Promise((resolve, reject) => {
      ...
      pipe.on('end', e => {
        ...
        console.log(`Copied ${snippetFileName}`.blue)
        if (upload) {
